...
export default function HeroSlider({ posts }: { posts: Post[] }) {
  ...
  const prev = useCallback(() => {
    ...
    setTimeout(() => {
      setCurrent((c) => {
        prevIndex.current = c
        return (c - 1 + posts.length) % posts.length
      })
      setVisible(true)
    }, 300)
  }, [posts.length])
  ...
  return (
    <div className="relative w-full h-[60vh] sm:h-[70vh] min-h-[400px] sm:min-h-[500px] max-h-[800px] overflow-hidden rounded-2xl bg-dark-900">

      {/* Background image — CSS transition replaces Framer Motion */}
      <div
      ...
      {/* Gradient overlays */}
      <div className="absolute inset-0 bg-gradient-to-r from-dark-950/95 via-dark-950/60 to-transparent pointer-events-none" />
      <div className="absolute inset-0 bg-gradient-to-t from-dark-950/80 via-transparent to-dark-950/20 pointer-events-none" />

      {/* Content — CSS transition replaces Framer Motion */}
